...
class MyCache(wharf.impl.Cache): # Then now you can create your subclass of ``wharf.impl.Cache``
    def __init__(self, http: wharf.HTTPClient): # This is needed but dw about having to access your bot classes http, this gets auto filled in internally!
...
bot = wharf.Bot(token="SomeToken", intents=wharf.Intents.MESSAGE_CONTENT, cache=MyCache) # You dont have to initalize the cache here as this is already done internally!

@bot.listen("ready") # Now you can do anything as usual, you can even subclass wharf.Bot, anything, it doesn't matter after you insert your cache into wharf.Bot!
async def ready():
    print("Im Ready!")
